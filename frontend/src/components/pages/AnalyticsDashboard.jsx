...
import { SidebarLink, SIDEBAR_LINKS } from "../admin/navigation";
...
const PRIVATE_NETWORK_LABEL = "Private / Internal Network";
...
const createRegionFormatter = () => {
  if (typeof Intl !== "undefined" && typeof Intl.DisplayNames === "function") {
    try {
      ...
      console.warn("Intl.DisplayNames initialization failed", err);
    }
  }
  return null;
};
...
const deriveGeoLabel = (row, formatter) => {
  ...
  if (country && city) {
    return { label: `${city}, ${country}`, country, city };
  }
  if (country) {
    return { label: country, country, city: null };
  }
  if (city) {
    return { label: `${city} (Unmapped Country)`, country: null, city };
  }
  return {
    label: PRIVATE_NETWORK_LABEL,
    country: null,
    city: null,
  };
};
...
const AnalyticsDashboard = () => {
  ...
  return (
    <div className="min-h-screen flex bg-gray-950 text-white">
      <aside className="hidden xl:flex flex-col w-64 bg-gray-900 border-r border-gray-800">
      ...
      <button
        className="xl:hidden fixed top-4 left-4 z-50 bg-gray-900/90 p-2 rounded-lg border border-gray-800"
      ...
      {sidebarOpen && (
        <aside className="fixed inset-y-0 left-0 z-40 w-64 bg-gray-900 border-r border-gray-800 flex flex-col xl:hidden">
      ...
      <div className="flex-1 flex flex-col min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-black">
        <div className="sticky top-0 z-20 bg-gray-950/85 backdrop-blur-lg border-b border-gray-800">
                ...
                <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-6">
                  ...
                  <div className="bg-black/40 border border-white/10 rounded-2xl p-6 shadow-xl w-full xl:w-80">
                    ...
                    <p className="text-white/60 text-sm mt-3 leading-relaxed">
                      {formatNumber(suspiciousTotals?.counterfeit)} flagged counterfeit
                      scans out of {formatNumber(dashboardTotals?.scanCount)} total
                                ...
                                {recentMoves.map((row) => (
                                  <tr key={row.id} className="hover:bg-white/5">
                                    ...
                                    <td className="py-3 px-4 text-white/70">
                                      {(row.from_owner_role || "unknown").toLowerCase()} → {(row.to_owner_role || "unknown").toLowerCase()}
                                    </td>
